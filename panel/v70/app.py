#!/usr/bin/env python3
"""iWAN Gateway v7.0.1: canonical iWAN schema and legacy migration."""
...
ORIGINAL_READ_JSON = core.read_json
ORIGINAL_APPLY = core.apply_config
AUTOSAVE_JS = core.WEB_DIR / "autosave.js"
AUTOSAVE_CSS = core.WEB_DIR / "autosave.css"


def compatible_read_json(path: Any, default: Any) -> Any:
    """Read sing-box configuration through the canonical iWAN schema adapter."""
    value = ORIGINAL_READ_JSON(path, default)
    if isinstance(value, dict) and iwan_compat.same_config_path(path, core.SINGBOX_CONFIG):
        return iwan_compat.migrate_config(value)
    return value


core.read_json = compatible_read_json
...
def _new_iwan(patch: dict[str, Any]) -> dict[str, Any]:
    normalized = iwan_compat.normalize_iwan_patch(patch)
    username = str(normalized.get("username", "")).strip()
    ...
    return {
        "type": "iwan",
        "tag": "iwan-in",
        "listen": str(normalized.get("listen") or "::"),
        "listen_port": int(normalized.get("listen_port") or 8000),
        "address_pool": str(normalized.get("address_pool") or "10.10.10.0/24"),
        "mtu": int(normalized.get("mtu") or 1400),
        "users": [{"username": username, "password": password}],
    }


def public_apply_config(payload: dict[str, Any]) -> tuple[bool, str]:
    """Normalize legacy keys, bootstrap the first inbound, and use the proven apply path."""
    normalized_payload = iwan_compat.normalize_payload(payload)
    current = core.read_json(core.SINGBOX_CONFIG, {})
    patch = normalized_payload.get("iwan")
    if not isinstance(current, dict) or not current:
        return False, "未找到有效 sing-box 配置"
    if not isinstance(patch, dict) or not patch or _has_iwan(current):
        return ORIGINAL_APPLY(normalized_payload)
    ...
    descriptor, temporary_name = tempfile.mkstemp(
        prefix="bootstrap.", suffix=".json", dir=str(core.SINGBOX_CONFIG.parent)
    )
    os.close(descriptor)
    ...
    try:
        ...
        ok, message = ORIGINAL_APPLY(normalized_payload)
        if ok:
...
def self_test() -> None:
    ...
    assert generated["address_pool"] == "10.10.10.0/24"
    assert "address" not in generated
    migrated = iwan_compat.migrate_config({
        "inbounds": [{"type": "iwan", "address": "10.20.0.1/16"}]
    })
    assert migrated["inbounds"][0]["address_pool"] == "10.20.0.1/16"
    assert "address" not in migrated["inbounds"][0]
    assert not _has_iwan({"inbounds": []})
    assert _has_iwan({"inbounds": [generated]})
    assert b"autosave.js" in page_html()
    print(json.dumps({"ok": True, "version": VERSION, "schema": "address_pool"}))
