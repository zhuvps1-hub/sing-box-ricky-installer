...
install_deps(){
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update -y
    DEBIAN_FRONTEND=noninteractive apt-get install -y python3 ca-certificates curl iproute2 tar xz-utils coreutils unzip gzip
    DEBIAN_FRONTEND=noninteractive apt-get install -y nftables || true
  elif command -v dnf >/dev/null 2>&1; then
    dnf install -y python3 ca-certificates curl iproute tar xz coreutils unzip gzip
    dnf install -y nftables || true
  elif command -v yum >/dev/null 2>&1; then
    yum install -y python3 ca-certificates curl iproute tar xz coreutils unzip gzip
...
install_mosdns(){
  ...
  if ! command -v mosdns >/dev/null 2>&1; then
    ...
    url="$(printf '%s' "$release_json" | ARCH="$arch" python3 -c 'import json,os,sys; d=json.load(sys.stdin); a=os.environ["ARCH"]; u=[x["browser_download_url"] for x in d.get("assets",[]) if "linux" in x.get("name","").lower() and a in x.get("name","").lower() and (x.get("name","").lower().endswith(".zip") or x.get("name","").lower().endswith(".tar.gz"))]; print(u[0] if u else "")')"
...
EOF
  ...
  if mosdns check -c /etc/mosdns/config.yaml >/dev/null 2>&1; then
    ...
    warn "现有 mosdns 配置校验未通过，已保留原配置，面板仍可用于修改。"
  fi
}
...
if [[ ${1:-} == "uninstall" ]]; then
  ...
  rm -f "$SERVICE_FILE" /etc/systemd/system/f7010u-gateway.service
  rm -rf "$APP_DIR" /opt/f7010u-gateway
  systemctl daemon-reload
...
# 从旧版 F7010U 目录迁移数据，但新版本不再使用该名称。
if [[ -d /etc/f7010u-gateway && ! -d "$CONFIG_DIR" ]]; then cp -a /etc/f7010u-gateway "$CONFIG_DIR"; fi
if [[ -d /var/lib/f7010u-gateway && ! -d "$DATA_DIR" ]]; then cp -a /var/lib/f7010u-gateway "$DATA_DIR"; fi
AUTH_EXISTS=0
...
systemctl disable --now f7010u-gateway 2>/dev/null || true
mkdir -p "$APP_DIR" "$CONFIG_DIR" "$DATA_DIR" /etc/sing-box/backups /etc/mosdns
chmod 700 "$CONFIG_DIR" "$DATA_DIR" /etc/sing-box/backups

info "下载 iWAN Gateway 完整版…"
...
# 去除旧设备品牌，并把内部目录迁移到 iwan-gateway。
find "$TMP_DIR/gateway" -type f \( -name '*.py' -o -name '*.html' -o -name '*.js' -o -name '*.css' -o -name '*.json' -o -name '*.md' \) -print0 | while IFS= read -r -d '' f; do
  sed -i \
    -e 's/F7010U iWAN Gateway/iWAN Gateway/g' \
    -e 's/F7010U/iWAN/g' \
    -e 's/f7010u-gateway/iwan-gateway/g' \
    -e 's/v2\.0\.0/v3.0.0/g' "$f"
done
rm -rf "$APP_DIR"; mkdir -p "$APP_DIR"; cp -a "$TMP_DIR/gateway/." "$APP_DIR/"
...
ProtectHome=true
ProtectSystem=full
ReadWritePaths=$CONFIG_DIR $DATA_DIR /etc/sing-box /etc/mosdns /usr/local/etc/mosdns /run /var/log
[Install]
WantedBy=multi-user.target
EOF
