...
→ 安装 mosdns v5
→ 创建 /etc/mosdns/config.yaml 通用默认配置
→ 启动 sing-box、mosdns systemd 服务
→ 安装 Web 面板
→ 设置面板登录账号和密码
```
...
- DNS 监听：`127.0.0.1:5335`（UDP + TCP）；
- API：`127.0.0.1:9091`；
- 缓存：10240 条；
- 默认上游：阿里 DNS、腾讯 DNS、Cloudflare、Google；
- 已存在 `/etc/mosdns/config.yaml` 时只备份并保留，不会覆盖。
...
如果 8088 被其他程序占用，安装器会自动选择后续空闲端口，并在安装结束时显示实际访问地址。
...
6. 打开 **DNS** 页面，可直接编辑、备份、恢复和重启已安装的 mosdns。
...
```bash
systemctl status sing-box mosdns iwan-gateway --no-pager
...
卸载面板、sing-box 和 mosdns 程序，但保留配置目录与备份：
