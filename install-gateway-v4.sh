...
info "下载 iWAN Gateway v4…"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT
for part in 00 01 02 03 04 05 06; do
  curl -fL --retry 3 --connect-timeout 15 \
    "$BASE_URL/gateway-v4.tar.xz.b64.part${part}" \
    -o "$TMP_DIR/part${part}"
done
cat "$TMP_DIR"/part* > "$TMP_DIR/gateway-v4.tar.xz.b64"
base64 -d "$TMP_DIR/gateway-v4.tar.xz.b64" > "$TMP_DIR/gateway-v4.tar.xz"
echo "17a3e1fabe8a0f3b3d704742213948ec4f57007ec2ffb33f145e98d537260e08  $TMP_DIR/gateway-v4.tar.xz" | sha256sum -c - >/dev/null || die "面板安装包校验失败。"
tar -xJf "$TMP_DIR/gateway-v4.tar.xz" -C "$TMP_DIR"
